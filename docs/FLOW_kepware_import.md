...
**三個狀態欄彼此獨立、沒有前後 gating**——程式不會阻止在未建點（`tb_status=pending`）時直接跑 Step 4 或 5，順序由使用者/呼叫端自行保證。<br>（Step 5 推導會回傳 `not_built_count` 並在畫面提示，但**刻意不阻擋**，因為「Kepware 上已手動建好點位」是合法用法。）
...
### ✅ 已修正（第二批，commit `262f648`，2026-07-26）
